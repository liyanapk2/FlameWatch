    ...
    if diff_value>40:
        ...
        if status:
            ...
            if res!=5:
                if status:
                    print(res,"==================================")
                    print(res,"animal detected,00000000000000000")
                    fn=datetime.datetime.now().strftime("%Y%m%d%H%M%S")+".png"
                    ...
                    # requests.get(f"http://127.0.0.1:8000/insertnotification?cid=1&fn={fn}")
                    # cv2.imwrite(r"C:\Users\msist\Downloads\project\homesecurityai\media/"+fn,frame3)
                    # res=requests.get("http://127.0.0.1:8000/insertnotification?fn="+fn+"&cida=1")
